        ...
        with open(Devices_path,'r') as f:
           for line in f:
               if line[0]=='#':
                   ...
                   info_dict={'ip':info_list[0],'username':info_list[1],'password':info_list[2],'su':info_list[3]}
...
def run_cmd(hosts,commands):
        ...
        try:
            net_connect = ConnectHandler(device_type='hp_comware_telnet',**hosts)  #telnet主机，输入账号密码
            ...
            for ip_cmd in commands:
                output = net_connect.send_command_timing(ip_cmd)
                time.sleep(1)
                if "Y/N" in output:
