    ...
    try:
        with open(CMD_path,'r') as f:
            for line in f :
                ...
                #    cmd_dict={'show':cmd_list[0],'run':cmd_list[1]}
                #    CMD.append(cmd_dict)
        return CMD
    except FileNotFoundError as e:
        print('找不到设备的配置文件')
...
def run_cmd(hosts=get_devices_info(Devices_path),commands=get_cmd_info(CMD_path)):
    ...
    for ip_host in hosts:
        try:
            net_connect = ConnectHandler(device_type='cisco_ios_telnet',**ip_host)  #telnet主机，输入账号密码
            ...
            for ip_cmd in commands:
                input = net_connect.send_config_set(ip_cmd)
            #   print(output)
                time.sleep(1)
